...
class eDynamoDB {
	...
	public function filter($attribute ="", $operator = "" , $criteria ="") {
		...
		if( (is_array($criteria) && sizeof($criteria) < 1) ) {
...
//		}
		...
		if(!isset($this->keys)) {
			...
			$query = new eDynamoQuery( array('num_rows' => sizeof($items[$table]), 'results' => $items[$table]));
	...
	public function put($table, $insert_array ="") {
		...
		if(!is_array($insert_array)) {
			...
			foreach($insert_array as $keys=>$row) {
                            $putRequest['PutRequest']= array();
                            $item = array();
                            foreach($row as $key=>$value){
				 $item[$key] = Attribute::factory($value);
                            }
                            $putRequest['PutRequest']['Item']= $item;
                            array_push($orders,$putRequest);
			}
                        $Items['RequestItems'][$table] = $orders;
		...
		if(!isset($this->key)) {
			throw new Exception('No key was selected.');
                ...
                if(!isset($this->keys)) {
                    ...
                    $Items['RequestItems'][$table] = array();
                    
                    foreach($this->keys as $key => $value) {
                        $attrib['DeleteRequest'] = array('Key' => $value);
                        
                        array_push($Items['RequestItems'][$table],$attrib);
...
class eDynamoQuery {
	...
	public function result() {
		...
		if($this->num_rows > 0 && sizeof($this->results)>0) {
...
//$dynamodb->put('Orders',$attributes);


//retrieve
$dynamodb->key('CustomerID',2,'OrderID',2);
$dynamodb->key('CustomerID',2,'OrderID',3);
...
//Scan
//$dynamodb->filter("CustomerID",'EQ',2);
//$dynamodb->get_attribute('CustomerID');
...
//$dynamodb->key('CustomerID',3,'OrderID',4);
